...
class DBHandlerTestClasses(unittest.TestCase):
    ...
    def test_insert_to_user_table(self):
        ...
        self.assertFalse(db_handler.insert_to_user_table(test5_name, test5_username, test5_password), "Test Failed")
    ...
    def test_search_entries_by_created_date(self):
        ...
        self.assertTrue(not (db_handler.search_entries_by_created_date(entry["Date_Created"]) == False), "Test Failed")

    def test_search_entries_by_modified_date(self):
        db_handler = DBHandlerClass()
        date = datetime.datetime.now()
        date += datetime.timedelta(days=1)
        self.assertFalse(db_handler.search_entries_by_modified_date(date), "Test Failed")
        entry = db_handler.support_func_get_all(1)[0]
        self.assertTrue(not (db_handler.search_entries_by_modified_date(entry["Last_Modified"]) == False),
                        "Test Failed")
    ...
    def test_update_entries(self):
        ...
        self.assertTrue(db_handler.update_entries(entry["_id"], entry), "Update Test Failed")
        self.assertFalse(db_handler.update_entries(ObjectId("111111111111111111111111"), entry), "Update Test Failed")

    def test_delete_entries(self):
        db_handler = DBHandlerClass()
        entry = db_handler.support_func_get_all(3)[0]
        self.assertTrue(db_handler.delete_entries(entry["_id"]), "Delete Test Failed")
        self.assertFalse(db_handler.delete_entries("111111111111111111111111"),
                         "Delete Test Failed")  # FIXME Check why this Fails
